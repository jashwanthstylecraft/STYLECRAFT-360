...
import CustomerServiceSummaryStrip from "../components/dashboard/CustomerServiceSummaryStrip";
...
const BASE_PATH = "/customer-service";

function PageHeader({ dateRangeLabel, isSampleData }) {
  return (
    <div className="mb-6 flex flex-wrap items-center gap-3">
      <div>
        <h1 className="text-2xl font-bold text-heading">Customer Service</h1>
        <p className="mt-1 text-sm text-ink-secondary">
          Returns and repair rate · <span className="font-medium text-ink">{dateRangeLabel}</span>
        </p>
      </div>
...
export default function CustomerService() {
  const { data, isLoading, isError, error } = useCustomerServiceMetrics();
  const dateRangeLabel = useDateRangeLabel();

  return (
    <PageShell lastUpdated={data?.asOf} scrollKey="customer-service">
      ...
      {isError && (
        <div className="mb-6 flex items-center gap-3 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-400">
          ...
          <div>
            <div className="font-semibold">Couldn't load customer service data</div>
      ...
      {!isError && (
        <>
          <div className="mb-6">
            {data ? (
              <CustomerServiceSummaryStrip summary={data.summary} />
            ) : (
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {data?.metrics.map((metric, index) => (
              <KpiCard
                ...
                weeks={data.weeks}
                basePath={BASE_PATH}
                departmentKey="customer-service"
