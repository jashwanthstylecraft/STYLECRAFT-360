...
const { wowDeltaPct, wowPointDelta, buildMetric, withLatestWeekSummary, withYtd, withRoc } = require("./metricsHelpers");
...
function getFinanceMetrics(period, range, opts = {}) {
  ...
  if (!opts.skipYtd) {
    metrics = withYtd(metrics, (ytdRange) => getFinanceMetrics("weekly", ytdRange, { skipYtd: true, skipRoc: true }));
  }
  if (!opts.skipRoc) {
    metrics = withRoc(metrics, (rocRange) => getFinanceMetrics("weekly", rocRange, { skipYtd: true, skipRoc: true }));
  }
  return {
    asOf: AS_OF,
    weeks: WEEKS,
